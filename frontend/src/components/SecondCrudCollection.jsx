...
const SecondCrudCollection = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await axios.get(`${URL}/second/items`);
  ...
  const addItem = async () => {
    ...
    try {
      await axios.post(`${URL}/second/add`, { name: item });
      setItem('');
  ...
  const addItems = async () => {
    try {
      await axios.post(`${URL}/second/add_many`, { items: [{ name: item }, { name: secItem }] });
  ...
  const deleteItem = async (id) => {
    try {
      await axios.delete(`${URL}/second/delete/${id}`);
      fetchItems(); // Перезагрузка элементов после добавления
    } catch (error) {
  ...
  const deleteItems = async () => {
    try {
      await axios.post(`${URL}/second/delete_many`, { ids: [id, secId] });
  ...
  const updateItem = async () => {
    try {
      await axios.put(`${URL}/second/update/${id}`, { name: newItem });
      fetchItems(); // Перезагрузка элементов после добавления
    } catch (error) {
  ...
  return (
    <div className={`${styles.flex_column}`}>
      <h1 className={styles.header_large}>Элементный Менеджер 2</h1>
...
export default SecondCrudCollection;
